...
void main() {
  group('HiveAtCommitLog iterate / replay', () {
    ...
    test('iterate yields every commit entry in commitId order', () async {
      ...
      await bundle.keyValueStore.commitLog!
          .commit('public:foo@alice', CommitOp.UPDATE);
      await bundle.keyValueStore.commitLog!
          .commit('public:bar@alice', CommitOp.UPDATE);
      await bundle.keyValueStore.commitLog!
          .commit('public:baz@alice', CommitOp.DELETE);
    ...
    test('iterate honours fromCommitId', () async {
      await bundle.keyValueStore.commitLog!
          .commit('public:a@alice', CommitOp.UPDATE);
      await bundle.keyValueStore.commitLog!
          .commit('public:b@alice', CommitOp.UPDATE);
      await bundle.keyValueStore.commitLog!
          .commit('public:c@alice', CommitOp.UPDATE);

      final all = await bundle.keyValueStore.commitLog!.iterate().toList();
      // Skip the first; keep the last two.
      final cutoff = all[1].commitId!;
      final partial = await bundle.keyValueStore.commitLog!
          .iterate(fromCommitId: cutoff)
          .toList();
    ...
    test('replay preserves the supplied commitId', () async {
      ...
      final entry = CommitEntry(
        ...
        ..commitId = 9001;
      await bundle.keyValueStore.commitLog!.replay(entry);

      final fetched = await bundle.keyValueStore.commitLog!.iterate().toList();
    ...
    test('replay is idempotent on the same (commitId, atKey, op)', () async {
      ...
      await bundle.keyValueStore.commitLog!.replay(entry);
      await bundle.keyValueStore.commitLog!
          .replay(entry); // second call is a no-op overwrite.
    ...
    test('replay rejects a CommitEntry without a commitId', () {
      ...
      expect(() => bundle.keyValueStore.commitLog!.replay(entry),
    ...
    test('iterate with where filter skips non-matching entries', () async {
      await bundle.keyValueStore.commitLog!
          .commit('public:alpha@alice', CommitOp.UPDATE);
      await bundle.keyValueStore.commitLog!
          .commit('public:beta@alice', CommitOp.UPDATE);
      await bundle.keyValueStore.commitLog!
          .commit('public:gamma@alice', CommitOp.UPDATE);

      final filtered = await bundle.keyValueStore.commitLog!
  ...
  group('Bundle clear', () {
    ...
    test('clear empties every store but keeps the bundle usable', () async {
      ...
      await bundle.keyValueStore.commitLog!
          .commit('public:before@alice', CommitOp.UPDATE);
      ...
      expect(bundle.keyValueStore.commitLog!.entriesCount(), 0);
      ...
      await bundle.keyValueStore.commitLog!
          .commit('public:after@alice', CommitOp.UPDATE);
      expect(bundle.keyValueStore.commitLog!.entriesCount(), 1);
    });
