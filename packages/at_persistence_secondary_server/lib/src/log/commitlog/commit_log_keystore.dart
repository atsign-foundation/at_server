...
class CommitLogKeyStore extends BaseCommitLogKeyStore {
  ...
  Future<Iterator<MapEntry<String, CommitEntry>>> getEntries(int commitId,
    ...
    int numberOfIndexsToIterate = getBox().length - startIndex;
    endIndex = startIndex + numberOfIndexsToIterate;
    ...
    try {
      ...
      if (sequenceNumber == -1) {
        ...
        while (startIndex <= endIndex) {
          var midIndex = (startIndex + endIndex) ~/ 2;
          CommitEntry commitEntry = await (getBox() as LazyBox).getAt(midIndex);
          if (commitEntry.commitId == sequenceNumber) {
            startIndex = midIndex;
            break;
          } else if (commitEntry.commitId == null || commitEntry.commitId! > sequenceNumber) {
            endIndex = midIndex - 1;
          } else {
            startIndex = midIndex + 1;
          }
