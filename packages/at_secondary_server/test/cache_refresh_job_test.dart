...
void main() {
  ...
  group('Cache refresh job', () {
    ...
    test('Ensure cron stopped by close()', () async {
      ...
      expect(
          () => closedCron!.schedule(Schedule(seconds: 1), () => print('hi')),
          throwsException);
    });
    ...
    test('Run with some records to refresh which have ttr > 0 not yet reached',
      ...
      AtCacheRefreshJob job = AtCacheRefreshJob(alice, cacheManager);
      Map result = await job.refreshNow();
      expect(result['keysChecked'],
          0); // ttr non-zero and we haven't waited (via await Future.delayed(Duration) e.g.) for refreshAt to be reached
      expect(result['valueUnchanged'], 0);
      expect(result['valueChanged'], 0);
      expect(result['deletedByRemote'], 0);
      expect(result['exceptionFromRemote'], 0);
    ...
    test(
        'Run with some records to refresh which have ttr > 0, some ready to refresh, exception from remote',
        () async {
    ...
    test(
        'Run with some records to refresh which have ttr > 0, some ready to refresh, no longer existing on remote',
        () async {
      DateTime past = DateTime.now().toUtc().subtract(Duration(seconds: 1));
      await createRandomKeyStoreEntry(
          '@bob', 'cached:@alice:1.key.app@bob', secondaryKeyStore,
    ...
    test(
        'Run with some records to refresh which have ttr > 0, some ready to refresh, some not, some changed, some not',
        () async {
      DateTime past = DateTime.now().toUtc().subtract(Duration(seconds: 1));
      await createRandomKeyStoreEntry(
          '@bob', 'cached:@alice:1.key.app@bob', secondaryKeyStore,
