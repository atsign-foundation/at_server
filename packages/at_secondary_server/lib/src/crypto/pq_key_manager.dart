...
/// signs it with this keypair via [signChallenge], and the verifier fetches
...
class PqKeyManager {
  ...
  /// this before calling [signChallenge]; if init never ran or failed, the
  ...
  Future<void> publishPublicKey(String atSign,
    ...
    _assertInitialized();
    final record =
        buildPqSigningPublicRecord({pqAlgoMlDsa65: _mlDsaPublicKey});
    await keyStore.put(
        pqSigningPublicKeyRecordName(atSign), AtData()..data = record);
    _log.info('Published PQ signing public key for $atSign');
  }

  /// Sign a handshake [challenge] with this server's ML-DSA secret key,
  /// returning the cookie value the verifier fetches at POL:
