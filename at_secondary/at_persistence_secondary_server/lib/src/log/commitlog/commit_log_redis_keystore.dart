...
class CommitLogRedisKeyStore implements LogKeyStore<int, CommitEntry> {
  ...
  Future add(commitEntry) async {
    ...
    try {
      ...
      internalKey =
          await redis_commands.send_object(['RPUSH', COMMIT_LOG, value]);
      logger.info('CommitLog InternalKey ${internalKey}, ${internalKey.runtimeType}');
      ...
      if (enableCommitId) {
        ...
        await redis_commands
            .send_object(['RSET', COMMIT_LOG, internalKey - 1, value]);
      }
    } on Exception catch (e) {
  ...
  Future<int> entriesCount() async {
    var totalKeys = 0;
    totalKeys = await redis_commands.send_object(['LLEN', COMMIT_LOG]);
    return totalKeys;
  }
  ...
  Future<CommitEntry> get(int key) async {
    try {
      var commitEntry;
      var value =
          await redis_commands.send_object(['LRANGE', COMMIT_LOG, key, key]);
      if (value == null) {
        return commitEntry;
      }
  ...
  Future<List> getFirstNEntries(int N) async {
    ...
    try {
      entries =
          await redis_commands.send_object(['LRANGE', COMMIT_LOG, 0, N - 1]);
    } on Exception catch (e) {
      throw DataStoreException(
  ...
  Future remove(int key) async {
    try {
      await redis_commands.send_object(['LREM', COMMIT_LOG, key]);
    } on Exception catch (e) {
  ...
  Future update(int commitId, CommitEntry commitEntry) async {
    try {
      ...
      await redis_commands.send_object(['LSET', COMMIT_LOG, commitId, value]);
    } on Exception catch (e) {
      throw DataStoreException('Exception updating entry:${e.toString()}');
    }
  }
  ...
  Future<List> getDuplicateEntries() async {
    var commitLogList =
        await redis_commands.send_object(['LRANGE', COMMIT_LOG, 0, -1]);
    var commitLogMap =
  ...
  Future<int> lastCommittedSequenceNumber() async {
    var lastCommittedSequenceNum =
        await redis_commands.send_object(['LLEN', COMMIT_LOG]);
