...
class InboundConnectionPool {
  ...
  void _checkWarningStatesOnAdd() {
    if (_connections.length >= _size * 0.85 && !passedEightyFivePercent) {
      logger.warning('InboundConnectionPool >= 85% full : ${_connections.length}/$_size');
      passedEightyFivePercent = true;
    }
    if (_connections.length >= _size * 0.95 && !passedNinetyFivePercent) {
      logger.severe('InboundConnectionPool >= 95% full : ${_connections.length}/$_size');
  ...
  void _checkWarningStatesOnRemove() {
    if (_connections.length < _size * 0.95 && passedNinetyFivePercent) {
      logger.info('InboundConnectionPool < 95% full : ${_connections.length}/$_size');
      passedNinetyFivePercent = false;
    }
    if (_connections.length < _size * 0.85 && passedEightyFivePercent) {
      logger.info('InboundConnectionPool < 85% full : ${_connections.length}/$_size');
